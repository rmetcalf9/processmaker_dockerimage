...
abstract class Base
{
    ...
    public function run($code, array $data, array $config, $timeout, ?User $user)
    {
        ...
        // *** RJM ADDING DOCKER login
        $docker_login_required = env('RJM_DOCKER_LOGIN_REQUIRED', 'false');
        if ($docker_login_required == 'true') {
            $cmd = Docker::command($timeout) . sprintf(
                ' login %s --username=%s --password=%s',
                env('RJM_DOCKER_REPO_URL'),
                env('RJM_DOCKER_REPO_USER'),
                env('RJM_DOCKER_REPO_PASS')
            );
            exec($cmd, $output, $returnCode);
            if ($returnCode !== 0) {
              throw new RuntimeException('Dockerlogin failed ' . $output);
            }
        };
        // *** END OF RJM CODE
